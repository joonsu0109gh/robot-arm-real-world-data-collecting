...
from model.utils.pose_repr_util import (
    compute_relative_pose, 
    convert_pose_mat_rep
)
from src.common.pose_util import (
    pose_to_mat, mat_to_pose, 
    mat_to_pose10d, pose10d_to_mat)
from model.model.common.rotation_transformer import \
    RotationTransformer
...
def get_real_obs_dict(
    ...
    for key, attr in obs_shape_meta.items():
        ...
        if type == 'rgb':
            ...
            this_audio_in = env_obs[key]
            obs_dict_np[key] = this_audio_in
        elif type == 'low_dim':
...
def get_real_umi_obs_dict(
    ...
    # debug every shape info
    for key, attr in obs_shape_meta.items():
        print(f"[Debug] obs key: {key}, attr shape: {attr.shape}")
    for key, val in env_obs.items():
        print(f"[Debug] env_obs key: {key}, val shape: {val.shape}")
