...
from src.common.timestamp_accumulator import get_accumulate_timestamp_idxs
...
class SingleRealsense(mp.Process):
    ...
    def __init__(
        ...
        self.min_d = 0.01  # meter
        self.max_d = 2.0  # meter
            ...
            while not self.stop_event.is_set():
                ...
                if self.video_recorder.is_ready():
                    if not self.enable_depth:
                        ...
                        depth_raw = rec_data['depth'].astype(np.float32)
                        depth_m = depth_raw * depth_scale

                        valid_mask = (depth_m > 0)

                        depth_clipped = np.clip(depth_m, self.min_d, self.max_d)

                        # normalize to 0–255
                        depth_norm = np.zeros_like(depth_clipped, dtype=np.uint8)
                        depth_norm[valid_mask] = (
                            (depth_clipped[valid_mask] - self.min_d) / (self.max_d - self.min_d) * 255
                        ).astype(np.uint8)

                        depth_colormap = cv2.applyColorMap(
                            depth_norm, cv2.COLORMAP_VIRIDIS
                        )
                        combined_frame = np.hstack((rec_data['color'], depth_colormap))
                        self.video_recorder.write_frame(combined_frame, 
                            frame_time=receive_time)
                    else:
                        raise RuntimeError('No color stream to record!')
                # perf
                t_end = time.time()
