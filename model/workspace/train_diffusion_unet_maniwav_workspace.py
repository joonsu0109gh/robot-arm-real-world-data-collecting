...
from model.policy.diffusion_unet_maniwav_policy import DiffusionUnetManiWAVPolicy
from model.dataset.base_dataset import BaseImageDataset, BaseDataset
from model.env_runner.base_image_runner import BaseImageRunner
from model.utils.checkpoint_util import TopKCheckpointManager
from model.utils.json_logger import JsonLogger
from model.model.diffusion.ema_model import EMAModel
from model.model.common.lr_scheduler import get_scheduler
...
def limit_threads(n: int = 1):
    print(f"[INFO] limit thread to {n}")
...
# %%
class TrainDiffusionUnetManiWAVWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf):
        ...
        # configure model
        self.model: DiffusionUnetManiWAVPolicy
        self.model = hydra.utils.instantiate(cfg.policy)

        self.ema_model: DiffusionUnetManiWAVPolicy = None
    ...
    def run(self):
        ...
        if accelerator.is_main_process:
            normalizer = dataset.get_normalizer(output_dir=self.output_dir)
        ...
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                def log_action_mse(step_log, category, pred_action, gt_action):
                    ...
                    # step_log[f'{category}_action_mse_error_z'] = torch.nn.functional.mse_loss(pred_action[..., 2], gt_action[..., 2])
...
def main(cfg):
    workspace = TrainDiffusionUnetManiWAVWorkspace(cfg)
    workspace.run()
