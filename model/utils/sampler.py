...
class SequenceSampler:
    def __init__(self,
        ...
        audio_keys: Optional[np.ndarray]=[],
    ...
    def sample_sequence(self, idx):
        ...
        obs_keys = self.rgb_keys + self.lowdim_keys + self.audio_keys
        if self.ignore_rgb_is_applied:
            obs_keys = self.lowdim_keys + self.audio_keys
        ...
        for key in obs_keys:
            ...
            if key in self.rgb_keys:
                ...
                # solve padding
                if output.shape[0] < this_horizon:
                    padding = np.zeros((this_horizon - output.shape[0], output.shape[1]))
                    output = np.concatenate([padding, output], axis=0)
            else:
